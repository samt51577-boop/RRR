        ...
        if (this.currentStakeDisplay) {
            ...
            if (potCard) {
                ...
                potCard.addEventListener('click', () => {
                    if (this.state.holeState.rolledBy || (this.state.holeState.presses && this.state.holeState.presses.length > 0)) {
                        alert("Cannot change base bet after action has started.");
                        return;
                    }
                    const newBase = prompt("Set New Base Bet ($):", this.state.settings.baseBet);
  ...
  renderFavorites() {
    ...
    this.favorites.forEach((favId) => {
      ...
      if (course) {
        ...
        row.innerHTML = `
>>>>>>> e322384d0c02ff30440e0d73eea9ca83b3717aa5
...
<<<<<<< HEAD
    ...
    startGame() {
        ...
        this.state.players = {
            p1: { name: getVal(this.inputs.p1, 'Player 1'), index: getIndex(this.inputs.p1Ghin), ch: 0, slope: p1Slp },
            p2: { name: getVal(this.inputs.p2, 'Player 2'), index: getIndex(this.inputs.p2Ghin), ch: 0, slope: p2Slp },
            p3: { name: getVal(this.inputs.p3, 'Player 3'), index: getIndex(this.inputs.p3Ghin), ch: 0, slope: p3Slp },
            p4: { name: getVal(this.inputs.p4, 'Player 4'), index: getIndex(this.inputs.p4Ghin), ch: 0, slope: p4Slp }
        };
    ...
    updateUI() {
        ...
        if (this.state.gameMode === 'roll') {
            ...
            if (this.state.holeState.reRolled) {
                this.multiplierBadges.innerHTML += '<span class="badge reroll">RE-ROLLED</span>';
            }
            if (pressTotal > 0) {
